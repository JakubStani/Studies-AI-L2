...
def AlphaBetaCalcAllHeuristicsAndReturnChildLeadingToBestHeuristic(node, minPlayer, maxPlayer, bestChildForThisNode):
    if not node._children()==None:

        #ta część będzie uruchomiona dla węzłów, które nie są liściami,
        #co oznacza, że w tej części można wykorzystać alfa beta cięcie
        bestChildForThisNode=None
        bestVal=None

        #Tut 20:06
        if minPlayer==str((node._round()%2)+1):
            #min, więc do szukania zaczynamy od najwyzszego
            bestVal=float('inf')
        else:
            #max, więc do szukania zaczynamy od najmniejszego
            bestVal=float('-inf')


        for child in node._children():
            #nad tym trzeba pomyśleć
            result=AlphaBetaCalcAllHeuristicsAndReturnChildLeadingToBestHeuristic(child, minPlayer, maxPlayer, bestChildForThisNode)
